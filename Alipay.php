...
class Alipay
{
    ...
    public $service_wap = 'alipay.wap.trade.create.direct';

    public $alipay_gateway_new = 'https://mapi.alipay.com/gateway.do?';
    public $alipay_gateway_mobile = 'http://wappaygw.alipay.com/service/rest.htm?';
    ...
    public function __construct($config, $is_mobile = false)
    {
        $this->config = $config;
     ...
     * 未考虑参数中空格被编码成加号“+”等情况
     */
    public function signParameters($params)
    {
        ...
        switch (strtoupper(trim($this->config['sign_type']))) {
            case 'MD5' :
                $result = md5($params_str.$this->config['key']);
                break;
            ...
            case '0001' :
                ...
                openssl_sign($params_str, $sign, $res);
     ...
     * @return <Array>
     */
    public function buildSignedParameters($params)
    {
        $default = array(
     ...
     * @return <String>
     */
    public function buildRequestFormHTML($params, $method = 'post', $target = '_self')
    {
     ...
     * @return <Array>
     */
    public function prepareMobileTradeData($params)
    {
    ...
    public function verifyCallback()
    {
        ...
        $sign_valid = $this->verifyParameters($data, $data['sign']);
        ...
        $response_txt = 'true';
        if (!empty($notify_id)) {
            $response_txt = $this->verifyFromServer($notify_id);
        }
        //验证
    ...
    public function verifyParameters($params, $sign)
    {
    ...
    public function verifyFromServer($notify_id)
    {
    ...
    {
        $pub_key = file_get_contents($ali_public_key_path);
    ...
    {
        $pri_key = file_get_contents($private_key_path);
